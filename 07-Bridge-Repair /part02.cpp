...
bool canFormTarget(const std::vector<long long>& nums, int currentIndex, long long currentValue, long long target) {
    ...
    std::string currentStr = std::to_string(currentValue);
    std::string nextStr = std::to_string(nums[currentIndex + 1]);
    long long concatenatedValue = std::stoll(currentStr + nextStr);

    if (canFormTarget(nums, currentIndex + 1, concatenatedValue, target)) {
        return true;
    }
