...
// Check if removing one element would make the sequence safe
bool canBeSafeByRemovingOneElement(const std::vector<int>& numbers) {
    for (size_t i = 0; i < numbers.size(); ++i) {
        std::vector<int> modified(numbers.begin(), numbers.end());
        modified.erase(modified.begin() + i); // Remove the i-th element
        
        if (isSorted(modified) && checkDifferences(modified)) {
            return true; // If the modified sequence is safe, return true
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    while (std::getline(inputFile, line)) {
        ...
        if ((isSorted(numbers) && checkDifferences(numbers)) || canBeSafeByRemovingOneElement(numbers)) {
